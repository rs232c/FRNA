...
class ArticleRenderer {
                            ...
                            return `
                                <a href="${article.url}" target="_blank" rel="noopener" class="group block">
                                    <div class="bg-gray-800 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300">
                                        ...
                                        ` : `
                                            <div class="relative aspect-square bg-gradient-to-br ${secSourceGradient} flex items-center justify-center">
    ...
    renderArticlesGrid() {
        ...
        // CRITICAL: Must have zip code
        if (!this.currentZip || !window.storageManager) {
            // Don't clear existing static HTML content - just return
            return;
        }
        
        // SPECIAL CASE: Don't render for default zip 02720 - use static HTML instead
        if (this.currentZip === '02720') {
            console.log('Skipping dynamic rendering for default zip 02720 - using static HTML');
            return;
        }
        
        // Additional safeguard: Don't clear grid if it already has static content
        // (Static HTML articles from server-side generation)
        if (gridContainer.children.length > 0 && !this.currentArticles.length) {
        ...
        gridArticles.forEach(article => {
            ...
            const articleHTML = `
                <article class="bg-gray-800 rounded-xl overflow-hidden shadow-lg border border-gray-700 flex flex-col h-full" data-category="${articleCategory.toLowerCase()}" data-primary-category="${articleCategory}">
                    ...
                    ` : `
                        <div class="relative aspect-square overflow-hidden bg-gradient-to-br ${sourceGradient} flex items-center justify-center">
