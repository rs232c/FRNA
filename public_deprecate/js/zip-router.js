...
class ZipRouter {
    ...
    doInit() {
        ...
        if (zipFromPath) {
            // Special case: 02720 is our default zip, treat it like root page
            if (zipFromPath === '02720') {
                // Clear any stored zip to prevent future redirects
                localStorage.removeItem('currentZip');
                console.log('At default zip 02720 - serving main city content like root page');
                return;
            }
            // Other zip codes: activate dynamic loading
